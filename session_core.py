...
def session_to_text(session: Dict[str, Any]) -> str:
    ...
    lines.append(f"Case Number: {session.get('case_number', '')}")
    lines.append(f"Examiner: {session.get('examiner', '')}")
    ...
    if playbook:
        ...
        for idx, step in enumerate(playbook.get("steps", []), start=1):
            ...
            lines.append(f"[{mark}] Step {idx}: {step.get('title', '')}")
            ...
            if notes:
                ...
                lines.append(notes)
            lines.append("")
    session_notes = (session.get("session_notes") or "").strip()
    if session_notes:
    ...
    lines.append(f"Checked Steps: {summary['checked_steps']} of {summary['total_steps']}")
    lines.append(f"Steps With Notes: {summary['steps_with_notes']}")
    return "\n".join(lines)


def save_session_outputs(settings: Dict[str, Any], session: Dict[str, Any]) -> Tuple[Path, Path, Path | None]:
